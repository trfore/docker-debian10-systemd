...
- Install [docker]
- Build an image locally (see above) or pull from Docker Hub: `docker pull trfore/docker-debian10-systemd:latest`
- Run a container from the image:

  ```sh
  docker run -d -it --name debian10-systemd --privileged --cgroupns=host --tmpfs=/run --tmpfs=/tmp --volume=/sys/fs/cgroup:/sys/fs/cgroup:ro trfore/docker-debian10-systemd:latest
  ```

- Use it, example:

  ```sh
  docker exec -it debian10-systemd /bin/bash
  ```
...
- Podman defaults to running containers in systemd mode, `--systemd=true`, and will mount the required tmpfs and cgroup
  ...
  ```sh
  podman run -d -it --name debian10-systemd docker.io/trfore/docker-debian10-systemd:latest
  ```
